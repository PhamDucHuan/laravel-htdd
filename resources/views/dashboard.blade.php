...
<div class="flex h-screen w-full bg-gray-100 overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-gray-50">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto p-6 md:p-8">
                        ...
                        <table class="min-w-full text-left text-sm whitespace-nowrap">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                <tr class="hover:bg-blue-50 transition-colors group">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-1 text-xs font-semibold rounded {{ $class->status == 'started' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                                            {{ $class->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <a href="{{ route('classrooms.show', $class->id) }}" class="text-slate-400 hover:text-blue-600 font-medium text-xs uppercase tracking-wide">Xem chi tiết &rarr;</a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
            ...
            {{-- ========================================================= --}}
            {{-- PHẦN DÀNH CHO GIÁO VIÊN (TEACHER) --}}
            {{-- ========================================================= --}}
            @if(Auth::user()->role === 'teacher')
                <div class="bg-white rounded-xl shadow-sm overflow-hidden border border-gray-100">
                    <div class="px-6 py-4 border-b border-gray-200 bg-blue-50 flex justify-between items-center">
                        <h3 class="font-bold text-blue-800 text-lg">Lớp học của tôi (Cần điểm danh hôm nay)</h3>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="min-w-full text-left text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100">
                                ...
                                @php
                                    $myClasses = \App\Models\Classroom::where('teacher_id', Auth::id())->where('status', '!=', 'closed')->get();
                                @endphp
